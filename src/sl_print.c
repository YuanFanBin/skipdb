...
static void printmetanode(FILE* stream, const char* prefix, metanode_t* mnode, uint64_t pos) {
    ...
    fprintf(stream, "%s[\033[36m%8llu\033[0m]: level = %d, flag = 0x%04x, offset = %llu, backward = %llu, value = %llu, forwards = [%llu",
        prefix,
        pos,
        mnode->level,
        mnode->flag,
    ...
    for (int i = 1; i < (int)mnode->level; ++i) {
        fprintf(stream, ", %llu", mnode->forwards[i]);
    }
    fprintf(stream, "], ");
}
...
void sl_print(skiplist_t* sl, FILE* stream, const char* prefix, int isprintnode) {
    ...
    fprintf(stream, "%s\033[34mmapsize = %lluB(%.2lfM), mapcap = %lluB(%.2lfM)\033[0m\n",
            prefix,
    ...
    if (sl->datafree != NULL) {
        ...
        while (lnode != NULL) {
            ...
            fprintf(stream, "[\033[36m%8llu\033[0m]]: offset = %llu, size = %d, data = ",
                    lnode->value,
                    dnode->offset,
                    dnode->size);
            printdatanode(stream, dnode);
